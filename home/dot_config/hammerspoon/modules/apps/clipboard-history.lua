...
---   file_authorities(clip_id, item_index, path) -- trusted local path snapshot
---   file_grants(token, item_index, path, created_ts, last_used_ts,
---               hard_expires_ts) -- short-lived bridge capabilities
...
local SENSITIVE_UTIS = {
  ...
  -- Public M mount enrichment writes standard file URLs plus this private
  -- marker. Skip that exact pasteboard change: its paths came from an
  -- unauthenticated peer pointer and must never become local file authority.
  ["org.chezmoi.clipboard.UntrustedFileURLs"] = true,
}
...
-- Path/format written by the dispatcher's O op:
-- <host>\n<sha256hex>\n<epoch>[\nsuppress-echo]. The optional 4th line is the
-- legacy one-shot echo-suppress flag written by the now-retired N op
...
-- O-op format.
-- Current mount enrichment instead carries the change-bound
-- org.chezmoi.clipboard.UntrustedFileURLs UTI and is skipped by
-- SENSITIVE_UTIS above. The flag parser remains for mixed-version cleanup.
...
local function ensure_schema()
  ...
  exec([[CREATE TABLE IF NOT EXISTS file_authorities (
    clip_id INTEGER NOT NULL,
    item_index INTEGER NOT NULL,
    path BLOB NOT NULL,
    PRIMARY KEY (clip_id, item_index)
  );]])
  exec([[CREATE TABLE IF NOT EXISTS file_grants (
    token TEXT NOT NULL,
    item_index INTEGER NOT NULL,
    path BLOB NOT NULL,
  ...
  exec("CREATE INDEX IF NOT EXISTS idx_file_grants_expiry "
...
-- are bound as BLOBs, never interpolated into SQL, so arbitrary filename bytes
-- (except NUL, which POSIX paths cannot contain) remain exact.
local function replace_file_authority(clip_id, paths)
  if not clip_id or not paths or #paths == 0 then return false end
  for _, path in ipairs(paths) do
    if type(path) ~= "string" or path:sub(1, 1) ~= "/"
       or path:find("/../", 1, true) or path:sub(-3) == "/.." then
      return false
    end
  end

  if exec("BEGIN IMMEDIATE;") ~= sqlite3.OK then return false end
  local del = db:prepare("DELETE FROM file_authorities WHERE clip_id=?;")
  if not del then exec("ROLLBACK;"); return false end
  del:bind(1, clip_id)
  local rc = del:step()
  del:finalize()
  if rc ~= sqlite3.DONE then exec("ROLLBACK;"); return false end

  for i, path in ipairs(paths) do
    local ins = db:prepare(
      "INSERT INTO file_authorities (clip_id,item_index,path) VALUES (?,?,?);")
    if not ins then exec("ROLLBACK;"); return false end
    ins:bind(1, clip_id)
    ins:bind(2, i)
    ins:bind_blob(3, path)
...
local function enforce_retention()
  ...
  exec("DELETE FROM file_authorities WHERE clip_id NOT IN (SELECT id FROM clips);")
  local now = now_ts()
  exec("DELETE FROM file_grants WHERE hard_expires_ts <= " .. now
    .. " OR last_used_ts < " .. (now - GRANT_IDLE_S) .. ";")
end
...
function M.capture_now()
  ...
  local authority_paths -- exact local paths this capture may expose through K
  ...
  if kind == "files" then
    ...
    if names and #names >= 2 then
      ...
      elseif names and #names == 1 then
        -- Keep a parseable standard file-list path authoritative even when
        -- readURL/stat could not refine its file-vs-directory kind.
        authority_paths = names
      end
    end
  end
  ...
  -- Mixed-version cleanup for an older mount-enrichment dispatcher that used
  -- the shared origin-file suppress flag. Current enrichment is rejected much
  -- earlier by its change-bound UntrustedFileURLs UTI.
  if not origin_host and (kind == "files" or kind == "file" or kind == "directory") then
    local idpaths = authority_paths
  ...
  if existing_id then
    ...
    up:step(); up:finalize()
    if not origin_host and authority_paths
       and not replace_file_authority(existing_id, authority_paths) then
      hs.printf("clipboard-history: failed to authorize local file capture %d", existing_id)
    end
    enforce_retention()
    return existing_id
  end
  ...
  if not origin_host and authority_paths
     and not replace_file_authority(new_id, authority_paths) then
    hs.printf("clipboard-history: failed to authorize local file capture %d", new_id)
  end
  enforce_retention()
  return new_id
end
