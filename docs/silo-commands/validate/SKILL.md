---
name: validate
description: Self-test and human-UX validation for /work-on-<silo> tasks. Mode A renders the worktree into a throwaway sandbox $HOME for parallel logic self-test (no lock, no clobber of real $HOME). Mode B is an agent-initiated UX session for changes needing eyeball judgment — serialized by a session lock, merges the branch to master and iterates live (the session IS the integration).
---
...
work (no separate `reconcile` skill step after).
...
4. **Merge your branch to master** using the same ff/divergence logic as
   the `reconcile` skill. If master diverged, the human resolves it **in the live
