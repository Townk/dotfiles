...
| chezmoi | chezmoi orchestration & run-scripts | `.setup.sh`, `.chezmoiscripts/`, `.chezmoi*.{tmpl,yaml}` | — | `run_onchange_*` ordering + hash-baking, `zellij-plugin-path.tmpl` shared resolver; `.setup.sh` two-axis bootstrap (platform × kind via profile-traits) |
...
- **`list [-u|--update] [-a|--all]` TSV rows** + `sync` (install declared / uninstall extras, strict where safe). Brew worker merges `Brewfile.bootstrap`+`Brewfile`, guards mise-owned runtimes (`^(go|python|node|...)@?`), tracks taps. Brew cleanup covers formulae/casks/taps only — App Store (`mas`) apps are **never** removed (installed set = Apple ID purchases + OS preinstalls, so undeclared ≠ unwanted; `mas uninstall` also needs root). Snap worker intentionally does **not** remove undeclared (snapd auto-installs base/platform snaps).
