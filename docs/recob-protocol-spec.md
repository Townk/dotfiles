...
**Q7's "OK" turned out to be conditional** (found live 2026-08-27): the answer
is truthful only when the process's FIRST `NSWorkspace` touch happens against
an established login session. The workspace's LaunchServices connection is
process-global and set up on first use; touched while the session is still
assembling, it latches the sessionless answer for the process's whole life. A
daemon started at boot — console login 11:19, recobd 11:19:25 — read
`frontmostApplication` as nil from then on and refused every observed copy as
the no-GUI-session echo guard for six days, silently, while `clip.get` and
every other pasteboard read kept working. The implementation therefore gates
all `NSWorkspace` use behind an out-of-process probe — `lsappinfo`, whose
fresh child process resolves the truth regardless of the parent's history —
and opens the gate only once a real, non-loginwindow application is frontmost,
deferring the first in-process touch until it is safe. The no-session refusal
also logs (content-free), so this failure mode can never be silent again.
