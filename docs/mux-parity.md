...
| …and it runs NO command at all: aimed at a client that already has a popup, `display-popup` returns 0, leaves the standing popup untouched and never starts the shell-command (measured 2026-07-29, two real clients) | a panel that arrives after a dialog cannot steal the screen — so the dialog needs no protection from a late panel, and the stale panel closes ITSELF (`_panel` exits on a stack that no longer names it) |
| the key table wins the panel's OPENING race: a key pressed before the popup is up reaches the table (the flip side of the row above — once the popup is up, only the panel sees keys) | both planes run the same keymap entry, so this is normally invisible; what is NOT invisible is a bind whose command and mode-stack op are two independent `run-shell -b` jobs |
| `display-popup -C` is client-scoped (`-c` is honored, verified against a second client) but cannot be aimed at a PARTICULAR popup, and the popup it closes makes its waiting client exit exactly **129** | `mux_stack::sync` names the client in the teardown, and a caller opening a popup of its own passes `MUX_STACK_KEEP_POPUP=1` (every `dialog` bind does) — without it the clear raced the dialog it was dispatched with and closed it: the clipboard picker's `tmux-popup … returned 129` |
| the mode stack is a TMUX-ONLY construct (zellij has native modes and a passive HUD) | none of the three rows above has a zellij half: zellij dialogs are floating panes, so nothing competes for a single popup slot |
