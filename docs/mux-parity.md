...
| `pty-frame` reconstructs the child's grid but ignored REP (`CSI Ps b`, "repeat the last graphic char n times") | bubbletea apps pad a row with one space + `CSI 8 b`, so the diff lens's filter box lost its padding and drew its right border straight after the label. Pre-existing (both backends), found via `pipe-pane` on the raw stream — capture-pane shows the RESULT, only the byte stream shows the cause |
