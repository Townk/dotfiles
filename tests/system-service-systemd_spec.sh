...
case "$*" in
  *" show "*)
    [ "${STUB_SHOW_RC:-0}" = 0 ] || exit "${STUB_SHOW_RC}"
    printf '%s\n' "$STUB_SHOW"
    ;;
  *"is-enabled"*)
...
STUB
  ...
  cleanup() {
    rm -rf "$TEST_TMP"
    unset STUB_SHOW STUB_SHOW_RC STUB_ENABLED STUB_CALLS
  }
  BeforeEach 'setup'
  AfterEach 'cleanup'
...
TOML
  ...
  Describe 'info (unreachable user manager)'
    It 'survives systemctl show failing and reports none'
      write_manifest
      export STUB_SHOW_RC=1
      When run zsh "$SYSTEMD_BIN" info demo
      The status should be success
      The output should include "com.system-service.demo.service"
      The output should include "status:   none"
    End
  End

  Describe 'render (full golden)'
    It 'renders the demo entry byte-exactly'
      write_manifest
      golden() {
        cat <<GOLD
# Generated by system-service-systemd from $SVCFILE — do not edit.
[Unit]
Description=demo daemon

[Service]
ExecStart="/bin/demo-daemon" "--flag" "a b"
Restart=always
...
GOLD
      ...
      When run zsh "$SYSTEMD_BIN" render demo
      The output should equal "$(golden)"
    End
  End
